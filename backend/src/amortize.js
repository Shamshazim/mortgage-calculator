...
/**
 * Amortization math for Amortize.
 * Implements the authoritative formulas from DESIGN.md, plus prepayment modeling
 * (extra monthly payments and one-time lump sums) with baseline-vs-accelerated savings.
 */
...
function validateInput(body) {
  ...
  // extraMonthlyPayment: optional, number, >= 0
  ...
  // extraLumpSum: optional, number, >= 0
 ...
 * loan is paid off. Each month pays the regular payment plus any extra monthly amount,
 * plus the one-time lump sum in its scheduled month.
 */
function buildAccelerated(principal, r, monthlyPayment, n, extraMonthly, lump, lumpSumMonth) {
  ...
  while (balance > 1e-9 && month < cap) {
    ...
    if (principalPaid < 0) principalPaid = 0; // guard (won't happen for valid loans)
    if (principalPaid > balance) principalPaid = balance; // don't overpay on the final month
...
/**
 * Compute the full amortization result for a validated input, including the
 * baseline schedule, the accelerated schedule (if any prepayments were supplied),
 * and the resulting savings.
 *
 * @param {{loanAmount:number, downPayment:number, annualInterestRate:number, termYears:number,
 *          extraMonthlyPayment?:number, extraLumpSum?:number, lumpSumMonth?:number}} input
...
module.exports = { validateInput, calculate, toNumber };
